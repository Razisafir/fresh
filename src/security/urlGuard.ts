...
export async function safeFetch(
        ...
        for (let hop = 0; hop <= maxRedirects; hop++) {
                ...
                if (response.status >= 300 && response.status < 400) {
                        ...
                        currentUrl = nextUrl;
                        // Strip body AND headers on redirect. Per fetch spec
                        // only the body is stripped, but for SSRF defence we
                        // also drop headers — prevents Authorization from
                        // leaking to a redirected host. `_headers` is
                        // intentionally unused (destructuring is the discard).
                        const { method, headers: _headers, ...rest } = init ?? {};
