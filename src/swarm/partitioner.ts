...
export class Partitioner {
         ...
         * Returns the repaired string (or the original if no repair applied).
         */
        private _repairJson(raw: string): string {
                let repaired = raw;
                // Remove markdown code fences (```json ... ``` or ``` ... ```)
                repaired = repaired.replace(/```(?:json)?\s*/gi, '').replace(/\s*```\s*$/g, '');
                // Remove trailing commas before } or ]
                repaired = repaired.replace(/,\s*([}\]])/g, '$1');
                // Close unclosed arrays — count [ vs ] and append missing ]
                const openArr = (repaired.match(/\[/g) ?? []).length;
                const closeArr = (repaired.match(/]/g) ?? []).length;
                if (openArr > closeArr) {
                        repaired += ']'.repeat(openArr - closeArr);
                }
                // Close unclosed objects — count { vs } and append missing }
                const openObj = (repaired.match(/\{/g) ?? []).length;
                const closeObj = (repaired.match(/}/g) ?? []).length;
                if (openObj > closeObj) {
                        repaired += '}'.repeat(openObj - closeObj);
                }
                return repaired;
        }
        ...
        private _parsePartitionResponse(text: string, plan: IApprovedPlan): IPartitionResult {
                ...
                try {
                        ...
                        logger.warn(`[Partitioner] Initial JSON parse failed (${reason}). Attempting repair…`);
                        jsonStr = this._repairJson(jsonStr);
                        try {
                                parsed = JSON.parse(jsonStr);
                        } catch (repairErr) {
                                const repairReason = repairErr instanceof Error ? repairErr.message : String(repairErr);
                                logger.warn(`[Partitioner] JSON repair also failed (${repairReason}). Falling back to single-agent.`);
                                return { shouldSwarm: false, reasoning: `Failed to parse partition JSON after repair: ${repairReason}`, subPlans: [] };
                        }
                }
                ...
                try {
                        const subPlansArr = p.subPlans as Array<{ agentId?: string; agentName?: string; stepIndices?: number[]; filesTouched?: string[]; description?: string }> | undefined;

                        if (!p.shouldSwarm) {
                                return { shouldSwarm: false, reasoning: (p.reasoning as string) ?? 'LLM recommended single-agent mode.', subPlans: [] };
                        }

                        if (!Array.isArray(subPlansArr) || subPlansArr.length === 0) {
                                return { shouldSwarm: false, reasoning: 'LLM returned shouldSwarm=true but no sub-plans.', subPlans: [] };
                        }

                        const subPlans: ISubPlan[] = subPlansArr.map((sp) => ({
                        ...
                        return {
                                shouldSwarm: true,
                                reasoning: (p.reasoning as string) ?? `Partitioned into ${validSubPlans.length} workers.`,
                                subPlans: validSubPlans,
                        };
                } catch (err) {
                        logger.warn(`[Partitioner] Parsed JSON structure invalid: ${err instanceof Error ? err.message : String(err)}`);
                        return { shouldSwarm: false, reasoning: `Failed to interpret partition JSON structure.`, subPlans: [] };
                }
        }
}
